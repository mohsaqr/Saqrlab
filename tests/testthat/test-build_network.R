...
test_that("zero-variance columns are dropped with message", {
  ...
  expect_false("constant" %in% colnames(net$network_matrix))
})
...
test_that("non-syntactic column names are dropped with message", {
  ...
  expect_false("%" %in% colnames(net$network_matrix))
})
...
# ---- Method: glasso (default) ----

test_that("build_network works with data frame input (glasso)", {
  df <- .make_freq_data(n = 80, p = 6)
  net <- build_network(df, nlambda = 20L)

  expect_s3_class(net, "psych_network")
  expect_equal(net$method, "glasso")
  expect_equal(net$n, 80)
  expect_equal(net$p, 6)
  expect_true(is.matrix(net$network_matrix))
  expect_equal(nrow(net$network_matrix), 6)
  expect_equal(ncol(net$network_matrix), 6)
  # Diagonal should be zero
  expect_true(all(diag(net$network_matrix) == 0))
  # Should be symmetric
  expect_equal(net$network_matrix, t(net$network_matrix))
  # Edges data frame
...
test_that("build_network works with correlation matrix input (glasso)", {
  ...
  net <- build_network(S, n = 100, nlambda = 20L)
...
test_that("build_network works with covariance matrix input (glasso)", {
  ...
  net <- build_network(C, n = 100, input_type = "cov", nlambda = 20L)
...
test_that("method aliases resolve to glasso", {
  df <- .make_freq_data(n = 80, p = 4)
  net1 <- build_network(df, method = "ebicglasso", nlambda = 20L)
  net2 <- build_network(df, method = "regularized", nlambda = 20L)
...
test_that("pcor errors on singular matrix", {
  ...
  S <- cor(mat)
  expect_error(
    build_network(S, n = 10, method = "pcor"),
    "singular"
  )
})
...
test_that("cor network_matrix matches thresholded cor_matrix", {
  df <- .make_freq_data(n = 80, p = 5)
  thr <- 0.15
  ...
  expect_equal(net$network_matrix, expected)
})
...
test_that("id_col columns are excluded from analysis", {
  ...
  net <- build_network(df, id_col = "subject_id", nlambda = 20L)

  # subject_id and rid should be excluded -> 5 variables
  expect_equal(net$p, 5)
  expect_false("subject_id" %in% colnames(net$network_matrix))
  expect_false("rid" %in% colnames(net$network_matrix))
})
...
test_that("higher gamma produces sparser or equal networks", {
  ...
  net_low <- build_network(df, gamma = 0, nlambda = 50L)
  net_high <- build_network(df, gamma = 1, nlambda = 50L)
...
test_that("print.psych_network produces expected output for glasso", {
  df <- .make_freq_data(n = 80, p = 5)
  net <- build_network(df, nlambda = 20L)
...
test_that("print.psych_network produces expected output for pcor", {
  df <- .make_freq_data(n = 80, p = 5)
  net <- build_network(df, method = "pcor")
...
test_that("print.psych_network produces expected output for cor", {
  df <- .make_freq_data(n = 80, p = 5)
  net <- build_network(df, method = "cor")
...
test_that("print.psych_network returns invisible(x)", {
  df <- .make_freq_data(n = 80, p = 5)
  net <- build_network(df, nlambda = 20L)
...
test_that("cor_method argument is respected", {
  ...
  net_pearson <- build_network(df, cor_method = "pearson", nlambda = 20L)
  net_spearman <- build_network(df, cor_method = "spearman", nlambda = 20L)
...
test_that("edges match non-zero upper triangle of network_matrix", {
  df <- .make_freq_data(n = 100, p = 6)
  net <- build_network(df, nlambda = 30L)

  mat <- net$network_matrix
...
test_that("level='between' aggregates to person means", {
  ...
  net <- build_network(df, id_col = "person", level = "between",
                        nlambda = 20L)
...
test_that("level='within' centers correctly (column means near 0)", {
  df <- .make_multilevel_data(n_persons = 30, obs_per_person = 5, p = 5)
  net <- build_network(df, id_col = "person", level = "within",
                        method = "cor")
...
test_that("level='both' returns psych_network_ml with both sub-networks", {
  df <- .make_multilevel_data(n_persons = 30, obs_per_person = 5, p = 5)
  net <- build_network(df, id_col = "person", level = "both",
                        nlambda = 20L)

  expect_s3_class(net, "psych_network_ml")
  expect_s3_class(net$between, "psych_network")
  expect_s3_class(net$within, "psych_network")
  expect_equal(net$method, "glasso")
...
test_that("level requires id_col", {
  ...
  expect_error(
    build_network(df, level = "between"),
    "id_col.*required"
  )
})
...
test_that("level requires data frame input", {
  m <- diag(5)
  expect_error(
    build_network(m, n = 50, id_col = "person", level = "between"),
    "data frame"
  )
})
...
test_that("level=NULL preserves backward compatibility", {
  df <- .make_multilevel_data(n_persons = 30, obs_per_person = 5, p = 5)
  net <- build_network(df, id_col = "person", nlambda = 20L)
...
test_that("print.psych_network shows level label for between", {
  df <- .make_multilevel_data(n_persons = 30, obs_per_person = 5, p = 5)
  net <- build_network(df, id_col = "person", level = "between",
                        nlambda = 20L)
...
test_that("print.psych_network shows level label for within", {
  df <- .make_multilevel_data(n_persons = 30, obs_per_person = 5, p = 5)
  net <- build_network(df, id_col = "person", level = "within",
                        method = "cor")
...
test_that("print.psych_network_ml shows both levels", {
  df <- .make_multilevel_data(n_persons = 30, obs_per_person = 5, p = 5)
  net <- build_network(df, id_col = "person", level = "both",
                        nlambda = 20L)
...
test_that("print.psych_network_ml returns invisible(x)", {
  df <- .make_multilevel_data(n_persons = 30, obs_per_person = 5, p = 5)
  net <- build_network(df, id_col = "person", level = "both",
                        nlambda = 20L)
  ret <- capture.output(result <- print(net))
...
test_that("predictability returns named numeric vector for glasso", {
  ...
  expect_equal(names(r2), colnames(net$network_matrix))
...
test_that("predictability.cor returns 0 for isolated nodes", {
  ...
  # Isolated nodes (no edges) should have R² = 0
  isolated <- vapply(seq_len(net$p), function(j) {
    all(net$network_matrix[j, ] == 0)
  }, logical(1))
  if (any(isolated)) {
...
test_that("predictability works for psych_network_ml", {
  df <- .make_multilevel_data(n_persons = 30, obs_per_person = 5, p = 5)
  net <- build_network(df, id_col = "person", level = "both",
                        nlambda = 20L)
  r2 <- predictability(net)
